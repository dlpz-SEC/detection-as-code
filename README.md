...
| | |
|---|---|
| Sigma rules on `main` | **7** — 3 production, 4 experimental |
| Production rules published to ADTE | **3** |
| Tooling tests | **136**, green |
| CI conversion targets | Splunk SPL, Microsoft Sentinel / Defender KQL |
| Live deploy paths | Wazuh Manager API, Sentinel ARM REST — both readback-verified |
| Lab | Bicep IaC: workspace, DCR, Windows event source, optional AD DS domain controller |

<sub>Measured from `main` at `49bdb01`, 2026-09-05. These are point-in-time; the live figures are
[`docs/COVERAGE.md`](docs/COVERAGE.md) and [`docs/rule_manifest.json`](docs/rule_manifest.json),
both regenerated by CI on every push to `main`.</sub>
